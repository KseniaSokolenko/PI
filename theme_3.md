...
### Вывод: 
С помощью input мы можем ввести значения переменных, а затем сравнить их с помощью >=. В зависимости от результата сравнения с помощью if, else будет выводится на экран 'выполняется' или 'Не выполняется'.
...
### Вывод: 
С помощью if, elif, else и сравнительных операций код может определять к какому промежутку относится заданная переменная.
...
### Вывод: 
Логический оператор in и позволяет нам определить находится ли число в заданном массиве. 
...
### Вывод: 
Логический оператор in и позволяет нам определить находится ли число в заданном массиве, а четность переменной можно преверить тем, делится ли она на 2 без остатка (%2).
...
### Вывод: 
Код выводит значения переменной i с помощью i in range() и с помощью условий if, elif, else.
...
### Вывод: 
С помощью value вводим букву, которую нужно найти, а затем с помощью index = string.find(value) находим порядковый номер этой буквы.
